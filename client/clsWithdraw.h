...
public:
	static void Show_Withdraw_screen()
	{
		...
		cout << setw(30) << left << "" << "\tEnter amount to withdraw : ";
		float amount = clsinput::read_Number<float>();
		if (amount > client.get_Account_Balance())
		{
			cout << setw(30) << left << "" << "\tInsufficient balance. Withdrawal cancelled." << endl;
			return;
		}
		cout << setw(30) << left << "" << "\tAre you sure you want to withdraw "
			<< amount << " from account " << account_number << " ? [Y/N] : ";
		char confirm = 'N';
		cin >> confirm;
		if (tolower(confirm) != 'y')
		{
			cout << setw(30) << left << "" << "\tWithdrawal cancelled." << endl;
			return;
		}
		client.Withdraw(amount);
		cout << setw(30) << left << "" << "\tAmount " << amount << " withdrawn successfully from account "
			<< account_number << "." << endl;
